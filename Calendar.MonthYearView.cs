    ...
    public partial class Calendar : ContentView
    {
        ...
        public void ShowMonths()
        {
            Device.BeginInvokeOnMainThread(() =>
            {
                ...
                var pcDate = StartDate.ToPersianDateTime();
                for (int r = 0; r < 4; r++)
                {
                    for (int c = 0; c < 3; c++)
                    {
                        var b = new CalendarButton
                        {
                            ...
                            Text = PersianDateInfo.Months[(r * 3) + c],
                            Date = new PersianDate(pcDate.Year, (r * 3) + c + 1, 1).ToDateTime(),
                        ...
                        b.Clicked += (sender, e) =>
                        {
                            ...
                            {
                                var dd = (sender as CalendarButton).Date.Value;
                ...
                for (int r = 0; r < YearsRow; r++)
                {
                    for (int c = 0; c < YearsColumn; c++)
                    {
                        ...
                        var b = new CalendarButton
                        {
                            HorizontalOptions = LayoutOptions.CenterAndExpand,
                            VerticalOptions = LayoutOptions.CenterAndExpand,
                            Text = string.Format("{0}", pcDate.Year + (t - (YearsColumn * YearsRow / 2))),
                            Date = new PersianDate(pcDate.Year + (t - (YearsColumn * YearsRow / 2)), pcDate.Month, 1).ToDateTime(),
                            BackgroundColor = DatesBackgroundColor,
                            TextColor = DatesTextColor,
                            FontSize = DatesFontSize,
                            FontAttributes = DatesFontAttributes,
