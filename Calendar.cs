...
using MauiPersianCalendar.Extensions;
...
namespace MauiPersianCalendar
{
    public partial class Calendar : ContentView
    {
        ...
        public static readonly BindableProperty StartDateProperty =
            BindableProperty.Create(nameof(StartDate), typeof(DateTime), typeof(DateTime), DateTime.Now,
        ...
        public DateTime StartDate
        {
            get
            {
                var date = (DateTime)GetValue(StartDateProperty);
                if (date.Date == DateTime.Now.Date && (SelectedDates != null && SelectedDates.Any()))
                {
                    return SelectedDates.LastOrDefault();
                }
        ...
        {
            if (newValue == oldValue) return;
            buttons.FindAll(b => b.IsEnabled && (!b.IsSelected || SelectedBackgroundColor != Colors.Black)).ForEach(b => b.BackgroundColor = newValue);
        }
        ...
        {
            if (newValue == oldValue) return;
            buttons.FindAll(b => b.IsEnabled && (!b.IsSelected || SelectedTextColor != Colors.Black) && !b.IsOutOfMonth).ForEach(b => b.TextColor = newValue);
        }
        ...
        {
            if (newValue == oldValue) return;
            buttons.FindAll(b => b.IsEnabled && (!b.IsSelected || SelectedTextColor != Colors.Black) && !b.IsOutOfMonth).ForEach(b => b.FontAttributes = newValue);
        }
        ...
        public PersianDate CalendarStartDate(PersianDate date)
        {
            var start = date;
        ...
        protected void CreateWeeknumbers()
        {
            ...
            for (var i = 0; i < ShowNumOfMonths; i++)
            {
                ...
                for (int r = 0; r < 6; r++)
                {
                    ...
                    weekNumbers.Children.Add(weekNumberLabels.Last());
                }
